...
Manual reading-priority decisions are maintained in `00-index/reading-plan-overrides.yaml`, not in generated Markdown files. After adding a new venue/year, update that YAML only when a B/C paper should be promoted to `P0` or `P1`, or when a paper should receive `advisor_track=yes`; then rerun the command above.
