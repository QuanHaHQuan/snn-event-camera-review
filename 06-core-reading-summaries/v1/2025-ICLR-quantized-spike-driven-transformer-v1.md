...
source_card: "01-papers-by-conference/ICLR2025/C/2025-ICLR-C-quantized-spike-driven-transformer.md"
...
tags: ["SNN", "quantization", "Transformer"]
---

# Summary V1｜Quantized Spike-driven Transformer

## 1. One-sentence Summary

本文围绕 Quantized Spike-driven Transformer，基于论文 PDF 中的方法与实验描述，总结其在 SNN 方法背景方向 的主要问题、方法和证据。
...
- Spiking neural networks (SNNs) have emerged as a promising approach for realizing energy-efficient
- despite their high energy efficiency, the application of SNNs is constrained by their low task accuracy.
- resulting in significant performance improvements on ImageNet benchmark (Zhou et al., 2023b;a;

本 V1 只引用这些可从 PDF 抽取到的实验线索；完整数值、baseline 顺序和显著性比较需要人工在 V2 中逐表核对。

## 7. Main Contributions

1. 提出或系统化研究了与 `Quantized Spike-driven Transformer` 对应的核心方法/任务设定。
2. PDF 摘要和方法部分给出了主要模块设计，涉及 SNN, quantization, Transformer。
...
理由：reading plan 将该论文标为 level C；PDF 内容显示其关键词和任务与 `SNN, quantization, Transformer` 相关。最终归类仍应在 V2 阶段结合全文细读修正。
...
- Local PDF parsed successfully: 2025-ICLR-quantized-spike-driven-transformer.pdf, 25 pages.
- PDF headings observed: ABSTRACT; 1 I NTRODUCTION; RectifyRectifyRectify; Information Enhanced LIF; Matrix Multiplication; Baseline; Transformer; Conv layer.
- PDF key experiment/evidence lines include datasets/metrics/table mentions listed in Section 6.

## 14. Draft Survey-Usable Sentences

Draft material: `Quantized Spike-driven Transformer` can be cited cautiously as C: SNN side background evidence after its quantitative tables and method details are manually verified.

Draft material: The paper is useful for mapping how SNN, quantization, Transformer relates to event-camera/SNN survey taxonomy, but V2 should refine the exact claim boundaries.
